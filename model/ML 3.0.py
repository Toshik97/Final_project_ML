...
def kill_dublicate0(df):  # удаляем явные повторения, чтобы сократить датасет.
    df = df.copy()
...
def extract_and_update_utm(df, page_path_col='hit_page_path'): # обработка параметров "utm" через URL
    df = df.copy()
...
def kill_nan_device(df): #убираем пропуски из устройств
    df = df.copy()
...
def group_rare_data(df, # сокращаем количество параметров
...
def main():
    ...
    models = (
        LogisticRegression(solver='liblinear', max_iter=1000, penalty='l1', C=0.1, class_weight='balanced'),
        RandomForestClassifier(max_depth=15, min_samples_leaf=2, class_weight='balanced'),
    ...
    with open('Model_balanced', 'wb') as file:
        dill.dump({
            'model': best_pipe,
            'metadata': {
                'name': 'Model target_action',
                'author': 'Natasha',
                'version': 1,
                'Comm': 'Models without preprocessing, using the built-in load balancer',
